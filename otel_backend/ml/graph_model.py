...
from otel_backend.ml.net import GATNet

GRAPH_MODEL = None

class GraphModel:
    def __init__(self):
        self.model = GATNet(num_node_features=1, num_edge_features=4, num_classes=2)
        self.optimizer = optim.Adam(self.model.parameters(), lr=0.01)
        self.trainer = IncrementalGraphTrainer(self.model, self.optimizer)

    async def train(self, new_data):
        data = await self.trainer.prepare_data(new_data)
...
#(Pdb) result = loop.run_until_complete(self.trainer.predict_anomaly_score(data))